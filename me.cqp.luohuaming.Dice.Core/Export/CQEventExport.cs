...
using me.cqp.luohuaming.Dice.Core;
using me.cqp.luohuaming.Dice.Core.Domain;
using me.cqp.luohuaming.Dice.Sdk.Cqp;
using me.cqp.luohuaming.Dice.Sdk.Cqp.Enum;
...
namespace me.cqp.luohuaming.Dice.App.Export
{
	/// <summary>	
	/// 表示酷Q事件导出的类	
	/// </summary>	
	...
	{	
		...
		{	
			return "9,me.cqp.luohuaming.Dice";	
		}	
		...
		{	
			...
			Type appDataType = typeof (AppData);	
			// 注册一个 CQApi 实例	
			AppInfo appInfo = new AppInfo ("me.cqp.luohuaming.Dice", 1, 9, "水银掷筛机", "1.0.0", 1, "落花茗", "R!", authCode);	
			appDataType.GetRuntimeProperty ("CQApi").GetSetMethod (true).Invoke (null, new object[] { new CQApi (appInfo) });	
			AppData.UnityContainer.RegisterInstance<CQApi> ("me.cqp.luohuaming.Dice", AppData.CQApi);	
			// 向容器注册一个 CQLog 实例	
			appDataType.GetRuntimeProperty ("CQLog").GetSetMethod (true).Invoke (null, new object[] { new CQLog (authCode) });	
			AppData.UnityContainer.RegisterInstance<CQLog> ("me.cqp.luohuaming.Dice", AppData.CQLog);	
